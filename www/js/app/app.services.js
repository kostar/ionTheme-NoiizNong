angular.module('your_app_name.app.services', [])
  ...
  .service('AuthService', function ($q, $http) {
    ...
    this.doLogIn = function (doLogIn) {
      var dfd = $q.defer();
      $http.post('http://localhost:3000/api/auth/signin', doLogIn).success(function (res) {
        dfd.resolve(res);
      }).error(function (err) {
        dfd.reject(err);
      });
    ...
    this.doSignUp = function (doSignUp) {
      var dfd = $q.defer();

      $http.post('http://localhost:3000/api/auth/signup', doSignUp).success(function (res) {
        dfd.resolve(res);
      }).error(function (err) {
        dfd.reject(err);
      });
  ...
  .service('faceService', function ($rootScope, $auth) {
    ...
    this.successAuth = function (success) {
      window.localStorage.setItem('user', JSON.stringify(success.data));
      $rootScope.$emit('userLoggedIn', { data: JSON.stringify(success.data) });
    };
    this.failedAuth = function (err) {
      $rootScope.$emit('userFailedLogin', JSON.stringify(err));
    };
